 ...
 */
  ...
  function main( $error ) {
    ...
    $res_cats_full = sql_query( "select jones_blog_category.id, name, parent, filename from jones_blog_category left join jones_binarydata on image = jones_binarydata.id", $dbi );
...
<?php
    $res = sql_query( "SELECT id, filename FROM jones_binarydata WHERE blogentry_id='0'", $dbi );
    while( list( $img_id, $img_name ) = sql_fetch_row( $res, $dbi ) ) {
?>
...
<?php
    ...
    while( list( $img_id, $img_name ) = sql_fetch_row( $res, $dbi ) ) {
?>
                    <option value="<?php echo $img_id; ?>" <?php if( $img_id == $category_image ) echo " selected"; ?>><?php echo $img_name;?></option>
<?php
    }
?>
                  </select>
...
<?php
  ...
  function upload( $id, $error ) {
  	global $dbi;
  	$res = sql_query( "SELECT id, filename, description, width, height, thumbnail FROM jones_binarydata WHERE blogentry_id=$id", $dbi );
    include 'header.php';
?>
    <div id="main">
      <div id="pageHeader">
...
<?php
    ...
    if (!($fp = @fopen( $image_file, 'rb'))) {
      ...
      case "image/gif":
        ...
        break;
      default:
        Header( "Location: ".$_SERVER['PHP_SELF']."?cmd=upload&id=$blog_id&error=1" );
        die( );
        break;        
     }
    ...
    $res = sql_query( "INSERT INTO jones_binarydata VALUES( NULL, '$name', '$size', '$type', '$now', '$description', '$data', '$width', '$height', '$thumbnail', '$blog_id' )", $dbi );
